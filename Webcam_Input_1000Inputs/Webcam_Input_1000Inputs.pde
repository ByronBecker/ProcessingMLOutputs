...
int numHoriz = 800/boxWidth;
int numVert = 600/boxHeight;
...
void draw() {
  ...
  for (int x = 0; x < 800; x += boxWidth) {
     for (int y = 0; y < 600; y += boxHeight) {
        float red = 0, green = 0, blue = 0;
        
        for (int i = 0; i < boxWidth; i++) {
           for (int j = 0; j < boxHeight; j++) {
              int index = (x + i) + (y + j) * 800;
  ...
  first = false;
  fill(0);
  text("Sending 1000 inputs to port 6448 using message /wek/inputs", 100, 100);
